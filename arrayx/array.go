...
func Combine[T comparable, E any](keys []T, values []E) map[T]E {
...
// Merge
// 数组合并
func Merge[T any](arr []T, options ...[]T) []T {
	if len(options) <= 0 {
		return arr
	}
...
// Slice
// 自定义位置截取
func Slice[T any](arr []T, start, length int) []T {
	if start > len(arr) {
...
// Diff 返回差集
func Diff[T comparable](array []T, otherArrays ...[]T) []T {
...
// Intersect 返回交集
func Intersect[T comparable](array []T, otherArrays ...[]T) []T {
	existMap := make(map[T]bool)
	for i := 0; i < len(array); i++ {
		existMap[array[i]] = false
	}
...
// Splice 从数组中移除选定的元素,并用新元素取代它
func Splice[T any](array []T, start, length int, replaceArray []T) []T {
	startArray := array[:start]
	endArray := Slice(array, start+length, len(array)-start+length)
...
// Sum 返回所有值的和
func Sum[T constraints.Float | constraints.Integer](array []T) T {
	var sum T
...
// Rand 从数组中随机抽取length个元素,返回新的数组
func Rand[T comparable](array []T, length int) []T {
...
func CountValues[T constraints.Ordered](array []T) map[T]int {
...
func Unshift[T any](array []T, element ...T) []T {
...
// Push 在数组结尾插入一个或多个元素
func Push[T any](array []T, element ...T) []T {
	array = append(array, element...)
	return array
}

// AscSort 升序排列
func AscSort[T constraints.Ordered](array []T) []T {
...
// DescSort 降序排列
func DescSort[T constraints.Ordered](array []T) []T {
	sort.Slice(array, func(i, j int) bool {
		return array[i] > array[j]
	})
	return array
...
func ToString[T any](array []T) string {
